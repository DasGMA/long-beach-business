...
export const checkForErrors = () => (dispatch, getState) => {
  ...
  if (userName.length < 2)
    dispatch(
      setLoginError("Username", "Username must be 2 or more characters."),
      (hasErrors = true),
    );

  if (password.length < 2)
    dispatch(
      setLoginError("Password", "Password must be 2 or more characters."),
      (hasErrors = true),
    );

  if (password !== verifyPassword)
    dispatch(
      setLoginError("verifyPassword", "Passwords do not match."),
      (hasErrors = true),
    );
